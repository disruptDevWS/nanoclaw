...
- Use `[PLACEHOLDER: description]` for any unconfirmed client data — never fabricate specifics. This applies equally to **geographic and factual specifics**: which towns belong to a county, named roads or highways, landmark names, ordinance or statute citations, distances, and response times. Do not state these from memory. State a geographic specific only when it appears in the brief, the schema, the client profile, or another verified input supplied to you. If a specific is not supplied, refer to the county or region generically, or flag `[PLACEHOLDER: verify local specifics]`. A confidently stated **wrong** geographic fact (e.g., placing a town in the wrong county) is worse than a generic reference a human editor will refine
