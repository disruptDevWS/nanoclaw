...
import { canonicalKeywordKey, STATE_ABBREV_TO_FULL } from '../src/lib/keyword-canonical.js';
import {
...
// Abbreviation → full name: imported from src/lib/keyword-canonical.ts
// (moved 2026-07-22 so the outreach verifier shares canonicalKeywordKey).
...
//
// canonicalKeywordKey (imported from src/lib/keyword-canonical.ts) is the
// ONE canonical key for every scout keyword comparison: within-set dedup of
// ranked + opportunity keywords, the gap-matrix cross-join, the
// recipient-facing variant collapse, and the outreach verifier's site match.
