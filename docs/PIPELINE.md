...
**Geographic-fact grounding + web-search verification (2026-07-10):** Oscar's content call (`generate-content.ts`) enables the server-side `web_search` tool, capped at 3 uses and restricted (`allowed_domains`) to `en.wikipedia.org` + `census.gov`. Oscar's system-prompt and playbook Dimension 3 now treat geographic/factual specifics (which towns are in a county, road names, ordinance numbers) as facts, not flavor: verify via web search against those trusted sources, else refer generically or flag `[PLACEHOLDER: verify local specifics]` — never assert from memory. The model only searches when the grounding rules require it, so pages with no geographic uncertainty incur zero searches. `webSearch.allowedDomains` was added to `callClaude` (`anthropic-client.ts`).
