...
4. Share-link ensure (2026-07-13): missing or **expired** `share_token` → mint a fresh UUID + `share_token_created_at` + `share_expires_at = now()+14d` so the drafted link can't be dead on arrival. Share URL = `{DASHBOARD_URL or https://app.forgegrowth.ai}/share/scout/{token}`
