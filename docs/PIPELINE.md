...
6. Persist copy to `prospects` (`outreach_status='generated'`) **before** the Gmail step
7. Gmail step (non-fatal): `getDelegatedUserAccessToken(sender, [gmail.compose])` → `createDraft`/`updateDraft` (`scripts/gmail-drafts.ts`) → on success `gmail_draft_id` + `outreach_status='drafted'`; on failure status stays `generated` and exit code is 0 (`--force` retries)
8. Manual lifecycle after send: the dashboard "Mark Outreach Sent" control flips `outreach_status='sent'` and restarts `share_expires_at` from the send. `viewed`/`booking_intent` arrive automatically via the share page (see DATA_CONTRACT prospects columns)
