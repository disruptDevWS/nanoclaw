...
- `scout-config` — writes prospect config to disk, triggers scout, reads reports via `/scout-report` (auth: `validateSuperAdmin` + `has_role`); public token-credential actions `get_share_report` (expiry check + view logging) and `log_booking_intent` (BOOKING_URL redirect) serve the share page
...
**Prerequisites:** `prospect-config.json` file with `name`, `domain`, `target_geos`, `topic_patterns`, `state`, and optional `estimated_job_value` (owner-provided typical job value in USD). No audit record required — uses `prospects` table instead.
...
4. **Deduplication** — Near-variant keywords collapsed via `canonicalKeywordKey()` (2026-07-20): ONE canonical key for every scout keyword comparison — state names/abbreviations dropped anywhere in the phrase, `&`→`and`, plural + `-ing`/`-er` stemming (`plumbers`/`plumber`/`plumbing` → `plumb`), alphabetical token sort. Applied to ranked keywords (best position wins) and opportunity map (highest volume wins) before gap matrix assembly; survivors carry `variant_count`. Embedding dedup (0.93 cosine) then catches paraphrases within each set and accumulates `variant_count` onto the representative.
5. **Gap matrix** — Cross-references rankings vs opportunity **by canonical key, not raw string** (2026-07-20 — the exact-string join produced phantom gaps: "furnace repair medford" reported as not-ranking while "furnace repair medford or" ranked #8). Canonical-key misses get a cross-set embedding pass (`matchOpportunitiesToRanked()`, same 0.93 threshold, non-fatal) so morphology/paraphrase variants join instead of surfacing as gaps a skeptical owner would spot. Ranked keywords consumed by an opportunity row (key or embedding match) are not re-added as standalone rows. Statuses: defending (1–10), weak (11–30), gap (not ranking). CPC backfill: $0 CPC entries filled from same-topic max, marked `cpc_inferred: true`. Inferred values shown with tilde prefix (`~$X`) in report tables.
6. **Revenue estimates** — Priority: owner-provided `estimated_job_value` (`method: owner_provided`) → **web-search-grounded market estimate** (`method: market_estimate`: `fetchMarketValueEstimate()` checks the `market_value_estimates` cache table per vertical+region (180-day freshness), else one Sonnet call with the Anthropic server-side web search tool (`web_search_20260209`, max 4 searches) returns `{acv_low/mid/high, cr, value_label, basis, sources}`; CR clamped to [0.005, 0.05]; result upserted so re-runs are deterministic and rows are hand-editable overrides) → `SCOUT_REVENUE_ESTIMATES` vertical benchmark (`method: vertical_benchmark`, only when the market lookup hard-fails) → **suppressed** (`revenue_assumptions: null`, no `rough_revenue_monthly`; share page and narrative fall back to CPC framing). `revenue_assumptions.basis` carries the plain-language source sentence to the share-page footnote and narrative prompt. `gap_summary.addressable_revenue_monthly` (2026-07-06) models monthly revenue if all gap/weak keywords reached page 1 — a conservative fit-ranking floor consumed by the dashboard's Addressable/mo column and poor-fit share warning; null when revenue is suppressed. The CPC×200 derived-ACV fallback was removed 2026-07-05 — it produced indefensible customer values.
7. **Top-opportunity hygiene** — `gap_summary.top_opportunities` (recipient-facing: share page "Revenue You're Missing" table + narrative) goes through: variant collapse via `canonicalKeywordKey()` (highest-volume variant kept as representative, volumes NOT summed, upstream `variant_count` accumulated) → out-of-state filter (full state names not in `target_geos`) → Sonnet relevance screen (`scout_opportunity_screen` phase: flags out-of-market locations, competitor brand names, non-service queries; non-fatal, keeps unscreened list on failure; refuses to drop 100%).
...
9. **Prospect narrative** — Sonnet generates 3-section outreach document. When revenue assumptions exist, prompt instructs ACV/conversion framing; when suppressed, prompt forbids inventing dollar figures and mandates CPC/volume framing. The model also emits a trailing `BRIDGE_LINE:` (2026-07-13) — a 1–2 sentence first-person seam from the prospect's sharpest finding to the engagement; `extractBridgeLine()` (`src/pipeline/bridge-line.ts`) strips it from the narrative and it persists to `prospects.prospect_bridge_line` (+ `prospect-bridge-line.txt` on disk). Absent/malformed → null, share page uses generic seam copy (non-fatal).
...
| Anthropic API (sonnet + server-side web search) | `callClaude({webSearch})` | Market-value estimate (cache miss only) |
...
3. **Write** — Brief saved to `audits/{domain}/research/{date}/strategy_brief.md` **and upserted to Supabase** (2026-07-09, migration 046): `audit_snapshots` row `agent_name='strategy-brief'`, `snapshot_version=1` (latest wins), `strategy_brief_markdown` column. The DB copy makes the brief available to runs that don't share the main service's `/app/audits` volume (cron services, local runs) and feeds Pam's context 7b fallback. Upsert failure is non-fatal (warning only)
...
- `audit_clusters.is_structural = true` rows (Michael-declared structural clusters, migration 044 — e.g. `service_area`, auto-created by syncMichael for declared Cluster Keys with no keyword backing) are likewise excluded from both DELETEs. They carry no keyword rows, so derived inserts never collide with them.
...
**SERP composition injection (B1 + C3, 2026-06-12):** runGap makes up to 12 targeted DataForSEO `live/advanced` SERP lookups (~$0.002 each, cost-logged) for the top opportunity keywords (non-brand, customer-intent, KD non-null; ranked by `delta_revenue_mid` then volume, max 2 per cluster — `selectSerpTargets()` in `src/agents/gap/serp-composition.ts`). Each top-10 is classified deterministically (weak_slot = forums/UGC/aggregators/directories list; direct_competitor = audit's `industry_competitor` domains; else authority) and produces an **effective KD**: ≥4 weak slots → ×0.7, ≤1 weak slots → ×1.3. Effective KD is compared against the proven ceiling (cluster ceiling from `computeProvenCeiling()`, site fallback, +5 headroom) → WITHIN_REACH / STRETCH / UNKNOWN verdicts. Video-carousel presence is detected from the same responses (C3 — no extra calls). Injected as `{{SERP_COMPOSITION_BLOCK}}`; raw data written to `research/{date}/serp_composition.json` (disk-first — no DB table). Entire block is try/catch non-fatal: missing creds, empty KD column (pre-036 audits), or per-keyword SERP failures degrade to no block. The Gap prompt's ranking rules require effective KD (not raw) for opportunity realism and STRETCH rationales to name the authority prerequisite.
...
- **Low-authority / thin-starter mode — REMOVED (2026-07-10):** formerly, a cold-start ceiling (<15 proven top-7 rankings) injected a LOW-AUTHORITY MODE block instructing Michael to add a `Mode` column (full | starter) and produce thin 200–400-word test pages plus individual `/faq/{question-slug}` stubs. This was removed: the operator standard is best-effort, comprehensive treatment on every page — the thin-starter tier saved only Oscar's word count (all upstream research/architecture cost is identical) while undercutting local/organic/AI visibility. The proven-ceiling **stretch-target sequencing** (bullet above) is a separate mechanism and is retained. FAQ/PAA intent now folds into on-page Q&A sections + FAQPage schema on the parent pillar/cluster (Michael system-prompt Rule 18), not standalone thin pages. The `page_mode` column and `scripts/detect-starter-expansion.ts` remain in place but are now inert (no page is ever emitted as `starter`).
...
- **Operator strategy directives (migration 044, 2026-07-08):** Supabase `audit_strategy_constraints` `active=true` rows → `OPERATOR STRATEGY DIRECTIVES` context block, injected LAST in `{{CONTEXT_BLOCKS}}` on EVERY run (not just re-runs). Framed as BINDING: overrides data-driven preferences; conflicts noted in the Executive Summary, never silently resolved. This is the durable operator channel for strategic corrections (e.g. county-level vs city-level geo pages).
- **Rejected recommendations (migration 044, 2026-07-08):** on re-runs, `execution_pages.operator_disposition IS NOT NULL` rows are listed in a `REJECTED RECOMMENDATIONS` do-not-re-propose table inside `{{RERUN_SECTION}}` with the operator's reasons — Michael must not re-propose these pages or near-equivalents, and must not list them as deprecation candidates. Disposed pages are also EXCLUDED from the COMMITTED ARCHITECTURE and MANUALLY ASSIGNED tables.
- **Previous architecture baseline (migration 044, 2026-07-08 — diff-mode default):** on re-runs, the prior synced architecture (`agent_architecture_pages` silo→pages + `agent_architecture_blueprint.executive_summary`, slim form — never full markdown) is injected as `PREVIOUS ARCHITECTURE BASELINE` inside `{{RERUN_SECTION}}`. The baseline is the DEFAULT: preserve unless current data justifies a change; every removal/re-home must be cited with a reason in the Executive Summary "Changes from Prior Architecture" paragraph. Read-timing invariant: these tables are replaced by Phase 6b, which always runs AFTER Phase 6 — do not reorder.
...
**Silo table columns:** URL slug, Status, Silo, Role, Coverage Role, Primary Keyword, Volume, Action, **Cluster Key** (migration 044 — REQUIRED, lowercase snake_case). Cluster Key declares each page's topic cluster: an existing keyword cluster's `canonical_key`, or a stable structural key (e.g. `service_area`) for silos with no keyword backing. The Coverage Role column uses the vocabulary: commercial, informational, geographic, comparison, faq, credential, outcome. Action column now includes AI visibility query mapping where applicable. `max_tokens` for the michael phase is 32768 (raised from 16384 in 2026-07-08 for the added re-run context).
...
**Geographic architecture (conditional by `geo_mode`):** Geographic rules are injected conditionally via `getGeographicArchitectureBlock()`. National mode = no geographic rules (topical architecture only). City/metro mode = service-primary container with city/metro geographic pages. State mode = service-primary container with state/city geographic pages driven by keyword data. All modes follow the principle: service is the primary topical container, location is the qualifier. **Granularity tier (2026-07-10):** both city/metro and state blocks now instruct Michael to choose the location tier — city, **county**, or state — by population/competition/keyword signal. County is the correct container for low-population/rural service areas where individual cities lack volume: one county page absorbs the intent and names the significant towns in-content, rather than spawning a thin page per town. This generalizes the former per-client operator directive (county-vs-city geo pages) into base geo knowledge. See `docs/prompts/michael-architecture-blueprint.md` for full block text.
...
| `strategic_rerun` | Committed pages get metadata-only update (page_brief, silo, priority). **Deprecated pages re-added by the blueprint are REVIVED to `not_started` (2026-07-08 `isCommitted` fix — `deprecated` is no longer "committed"; `published_at` still protects).** Stale uncommitted pages set to `deprecated`. Parses `## Deprecation Candidates` JSON from blueprint for explicit deprecation. |
...
**operator_disposition (migration 044, 2026-07-08):** In every scenario, `execution_pages.operator_disposition IS NOT NULL` rows (operator rejected/deferred the recommendation via the dashboard) are IMMOVABLE — only `snapshot_version` is bumped; they are never revived, re-homed, brief-updated, stale-deprecated, or deprecation-candidate-deprecated. The disposition guard runs BEFORE the assignment_locked guard in the page loop. The canonical_key backfill also filters `.is('operator_disposition', null)`.

**Declared Cluster Key + structural clusters (migration 044, 2026-07-08):** The parser extracts the blueprint's `Cluster Key` column into `ArchPage.cluster_key` (detector runs before the silo/cluster detector; normalized to snake_case; backward compatible — absent column = old behavior). At seed time, syncMichael: (1) upserts an `audit_clusters` row (`is_structural=true`, `status='inactive'`) for every declared key with no existing cluster; (2) writes declared keys to `execution_pages.canonical_key` (`cluster_active` mirrors the cluster's current status) in all upsert branches except locked/disposed; (3) the primary-keyword backfill runs only for pages WITHOUT a declared key and never overwrites one.
...
6. `architecture_blueprint.md` — silo excerpt from disk; **Supabase fallback** (2026-07-09): if the disk file is absent, the latest `agent_architecture_blueprint.blueprint_markdown` for the audit is used with the same silo-excerpt logic. (Disk-absence reality, corrected 2026-07-09: the MAIN Railway service persists `audits/` via the `nanoclaw-volume` at `/app/audits`; the cron services have no volume, and local runs lack Railway-generated artifacts — the fallbacks cover those cases)
...
**Starter page directive — RETIRED (2026-07-10):** with thin-starter mode removed (see the Michael Low-authority note), Michael no longer emits `Mode=starter`, so no brief carries `page_mode='starter'` and the binding Starter Page Directive path is dormant. The brief-generation branch (`generate-brief.ts`) and Oscar's starter-ceiling language (playbook §4 + system-prompt exceptions) remain but are unreachable in practice. The Bottom-of-Funnel Length Directive is unaffected and still binding.

**Geographic-fact grounding (2026-07-10, revised same day):** Oscar's system-prompt and playbook Dimension 3 treat geographic/factual specifics (which towns are in a county, road names, ordinance numbers) as facts, not flavor: state them only from verified inputs (brief, schema, client profile), else refer generically or flag `[PLACEHOLDER: verify local specifics]` — never assert from memory. **Oscar's generation call is deliberately tool-free**: giving it the server-side `web_search` tool (tried same day) grounded correctly but flipped the model into agentic narration — it described its searches and "exported a file" instead of emitting the HTML artifact — so the tool was reverted. `webSearch.allowedDomains` support remains in `callClaude` (`anthropic-client.ts`) for other phases; the planned robust path is a pre-generation grounding step that feeds verified locale facts INTO the brief (not yet built — see DECISIONS.md 2026-07-10).
...
### detect-starter-expansion.ts (D1 lifecycle) — INERT since 2026-07-10

> Thin-starter mode was removed 2026-07-10; no page is emitted as `page_mode='starter'`, so this script now always reports zero starter pages. Retained for history / possible future use, not part of the live flow.
...
**CLI:** `npx tsx scripts/generate-outreach-email.ts --domain <domain> [--force] [--approve-flagged]`
...
4. Share-link ensure (2026-07-13): missing or **expired** `share_token` → mint a fresh UUID + `share_token_created_at` + `share_expires_at = now()+14d` so the drafted link can't be dead on arrival. Share URL = `{DASHBOARD_URL or https://app.forgegrowth.ai}/share/scout/{token}` **+ UTM attribution** (outreach tracking, 2026-07-21, `buildShareUrl`): `?utm_source=outreach&utm_medium=email&utm_campaign=scout[_{vertical}]&utm_content={variant_id}[&utm_term={market}]`. `utm_content=variant_id` is the fitness-function join key (placeholder = `outreach_variant` until a genome population exists); `utm_campaign=scout_{vertical}` from `prospects.vertical` (migration 049 — cron-prospector stamps it post-scout, dashboard form sets it, backfilled from candidates), base `scout` when null; `market` from `target_geos[0]`. Params ride the single URL, so the one-link rule holds. See `docs/plans/FORGE_OS_OUTREACH_TRACKING_SPEC.md`
5. One Sonnet call (`phase: outreach_email`, 2048 max tokens), `---SUBJECT---`/`---BODY---` markers; missing markers → exit 1, nothing written. Prompt carries the grounding rule (cite only provided facts), the em-dash STYLE RULES block, and the **one-link rule** (2026-07-13, reverses the original no-links rule): the share URL exactly once, plain text, framed as their prepared report; no other URLs. Precondition for actually sending: SPF + Gmail DKIM published for forgegrowth.ai (root SPF and google._domainkey were missing as of 2026-07-13 — Matt's DNS task)
6. **Claim verification (Bucket B, 2026-07-22)** — `verifyDraft()` (`src/agents/outreach-verify/`, spec: `docs/plans/FORGE_OS_OUTREACH_VERIFIER_SPEC.md` §3) runs **before anything persists or reaches Gmail**: grounded claim extraction (one Sonnet call, `phase: outreach_claim_extract`, constrained to scope.json services/locales — asserted text must be a verbatim substring, fail loud) → site inventory (robots → sitemap → homepage nav, cheerio fetch+parse; **only a sitemap-backed inventory can mint ABSENT**; no Playwright — JS-only nav flags to manual, approved deferral) → `canonicalKeywordKey` match → verdicts (`PRESENT`/`ABSENT`/`UNRESOLVABLE`) → deterministic subtraction (PRESENT pure-absence → sentence cut; PRESENT ambiguous-presence → whitelist soften, **never a ranking rewrite**) → coherence gate (word floor; gutted drafts route to review) → disposition `clean`/`weakened`/`needs_review`/`killed`. Verdict log: `audits/{domain}/outreach/{date}/verification.json` + jsonb mirror `prospects.outreach_verification_json` (migration 050)
7. Persist copy to `prospects` **before** the Gmail step — vetted body, `outreach_status='generated'` (clean/weakened) | `'needs_review'` | `'killed'` (killed keeps the original body for review)
8. Gmail step (non-fatal, **disposition-gated**): only `clean`/`weakened` drafts reach Gmail — the draft is created from the vetted body, never create-then-edit, so no unvetted draft ever exists in Gmail. `needs_review`/`killed` → no draft; resolve via `--force` (regenerate + re-verify) or `--approve-flagged` (materialize the stored copy as-is — Matt's explicit override, the only flagged→drafted path). On Gmail failure status stays as persisted and exit code is 0 (`--force` retries)
9. Manual lifecycle after send: the dashboard "Mark Outreach Sent" control flips `outreach_status='sent'` and restarts `share_expires_at` from the send. `viewed`/`booking_intent` arrive automatically via the share page (see DATA_CONTRACT prospects columns)
...
`contact_email`, `contact_name` (manual entry), `outreach_subject`, `outreach_body` (source of truth — the Gmail draft is a materialization; post-verifier this is the **vetted** body), `outreach_variant` (`pitch|courtesy_note`), `outreach_status` (`none|generated|needs_review|killed|drafted|sent`), `gmail_draft_id`, `outreach_generated_at`, `outreach_verification_json` (verdict-log mirror).
**Migrations:** `scripts/migrations/043-prospect-outreach.sql`, `050-outreach-verification.sql`
...
**Config:** `config/prospector-seeds.json` — verticals (medical academy, trade school, HVAC, plumbing, electrical, roofing, pest control) × geos × query templates, plus caps (`daily_query_cap` 12, `daily_qualify_cap` 15, `daily_scout_cap` 3, `min_qualify_score` 60, SERP rank window 11–50) and exclusions (aggregator/franchise domain blocklist, `.gov/.edu/.mil/.org` TLDs, Boise/Meridian domain-substring guard). **Footprint (Matt, 2026-07-09):** Idaho incl. Nampa/Caldwell, eastern OR (Bend/Pendleton/La Grande/Baker City), eastern WA (Spokane/Kennewick/Yakima/Walla Walla), western MT, western WY, northern UT excluding Salt Lake City. **Boise and Meridian are hard-excluded** (saturated agency market; three layers: seed geos, domain substrings incl. `treasurevalley`, qualifier hard-zero + config-validator leak check). Nampa/Caldwell were un-excluded from the original Treasure Valley blanket ban 2026-07-09. Seeds are the footprint source of truth: backlog and qualified-pool pulls filter `geo->>metro` against current seed metros, so candidates from removed metros stay buried (revive by re-adding the metro or manual status flip).
...
6. **Outreach draft** — awaited spawn of `generate-outreach-email.ts` (5-min timeout); variant/idempotency/verification/Gmail logic unchanged from the Outreach section. The claim verifier runs in-process there, so a flagged draft reaches the digest as flagged with **no Gmail draft behind it** — the digest can never surface an unvetted draft
7. **Digest** — drafts-ready list (subject, variant, addressable, To:, claim-verifier disposition), a **FLAGGED BY CLAIM VERIFIER** section (`needs_review`/`killed` prospects with their claim→verdict lines and the `--force`/`--approve-flagged` resolution commands — this is the manual queue), generated configs for spot-checking, other candidates with scores/reasons; full dump to stdout (Railway logs) + Gmail draft addressed to the sender (`gmail.compose` only — no send scope exists)
