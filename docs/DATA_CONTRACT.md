...
| `vertical` | cron-prospector (post-scout, from seed) + Dashboard (New Prospect form) | Pipeline (generate-outreach-email.ts) | Migration 049 (2026-07-21). Vertical slug (plumbing/hvac/electrical); drives `utm_campaign=scout_{vertical}` on outreach share links. Backfilled from `prospect_candidates.vertical`. Nullable → falls back to base `scout` |
