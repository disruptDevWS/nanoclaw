...
**(3) Geo facts self-grounded via web search, not human roster.** The fix for the Cambridge-class error is deliberately *not* an operator-supplied town roster (that is the human buffer we are trying to eliminate). Instead Oscar's content call enables the server-side `web_search` tool restricted to trusted sources (`en.wikipedia.org`, `census.gov`, max 3 uses), and its prompt + playbook Dimension 3 reclassify geographic/factual specifics (town-in-county, road names, ordinance numbers) as facts requiring verification: verify via search, else refer generically or flag — never assert from memory. Dimension 3 previously *rewarded* confident local specificity and even modeled an invented ordinance citation ("Ada County Code 3-2-1"); that example is now caveated as correct only if the citation is real/verified. The model searches only when the grounding rule fires, so non-geographic pages incur no search cost. `webSearch.allowedDomains` was added to `callClaude`. FAQ/PAA intent (formerly individual thin `/faq/` starter pages) now folds into on-page Q&A + FAQPage schema on the parent page (Michael Rule 18).
