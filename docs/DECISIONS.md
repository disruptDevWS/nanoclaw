...
- **`utm_campaign` is base `scout`, not `scout_{vertical}`, because `prospects` has no `vertical` column** (it lives only on discovery candidates). `buildShareUrl` takes an optional `vertical` (currently null → `scout`). Add a real vertical to the prospect row to light up per-vertical campaigns. **[RESOLVED 2026-07-21, migration 049]** — `prospects.vertical` added: cron-prospector stamps it post-scout from the seed vertical, the dashboard New Prospect form sets it for manual prospects, and existing prospects were backfilled from `prospect_candidates.vertical`. `buildShareUrl` now reads `row.vertical`, so campaigns are `scout_{vertical}` when known.
...
### 2026-06-11 — Empty-rebuild guard + strategy regeneration reactivates (Weiser "stuck strategy" bug)

**Symptom → root cause.** Weiser /clusters showed "Strategy not yet generated or still processing..." despite a freshly regenerated 30KB strategy. The row existed but had `status='deprecated'`, and the dashboard filters `.eq('status','active')`. Two compounding bugs:

1. **`rebuildClustersAndRollups()` ran destructively on an empty keyword set.** syncJim calls the rebuild BEFORE Phase 3c canonicalize assigns `canonical_key`, so on every full re-run the jim-pass rebuild sees 0 canonicalized keywords → deletes all clusters → treats every active strategy as "orphaned" and deprecates it. The recanonicalize rebuild ~20s later restores clusters but NOT strategy status or activation state (its statusMap snapshot is taken after the jim pass already wiped them). This was systematic for re-runs, not a race: it only surfaced now because Weiser (2026-06-11 00:30 end-to-end run) was the first full re-run over an audit with an active strategy. Audit trail: `agent_runs` warning `deprecated_cluster_strategies` with `surviving_cluster_keys: []`.
2. **Regenerating a strategy couldn't un-deprecate it.** The `generate-cluster-strategy.ts` upsert didn't set `status`/`deprecated_at`, so a conflict-update on a deprecated row kept `deprecated` even with fresh markdown — the cluster got reactivated (step 11) but the strategy stayed invisible.

**Fix: bail out before deleting, and regeneration always reactivates.** The rebuild now fetches keywords BEFORE the cluster delete and returns early on 0 canonicalized rows, leaving clusters/rollups/activations/strategies untouched (logged to `agent_runs` as `skipped_empty_cluster_rebuild` when prior clusters existed). Rationale: an empty set means "keywords not canonicalized yet," never "every cluster ceased to exist" — Phase 2's QA gate already fails the pipeline on genuinely-zero keywords, so a legit empty state shouldn't reach sync; and stale clusters are strictly less harmful than destroyed activation/strategy state. The strategy upsert now sets `status='active'`, `deprecated_at=NULL` — a human-triggered regeneration is an explicit statement that the strategy should be live. Alternative considered and rejected: only guarding the deprecation block (leaving the empty delete in place) — that still wipes activation state across sync passes.

**Data repair applied:** Weiser `towing_services` strategy row flipped back to `active` manually. Two forgegrowth.ai rows (`local_seo`, `ai_search_optimization`, April 2026) show the same deprecated-with-existing-cluster shape but were deliberately left deprecated: their clusters are `inactive`, nothing displays them, and the strategies are stale — regenerating via the dashboard now reactivates them automatically if ever wanted.

---

### 2026-06-12 — KB extraction Phase 0 + Workstream A (KD persistence, proven ceiling, re-eval, zero-click fan-out)

**Source.** First implementation session from the knowledge-base extraction map (`docs/research/kb-extraction-map.md`, source repo `/home/forgegrowth/claude-code-seo/`). Scope deliberately limited to Phase 0 (KD column) + Workstream A (read-only analytics) — prompt-touching workstreams (B/C) wait for threshold tuning and the planned Gap revision session. The 2026-04-09 leak-material boundary holds: no T*/Q*/P* signal weights anywhere near agent prompts; these scripts consume only first-party (GSC) and already-licensed (DataForSEO) data.

**KD backfill pulls artifacts from Railway, merged across dates.** `audit_keywords` rows are written by Railway runs, but local `audits/` only mirrors local runs — IMA's local 2026-03-12 artifact matched just 536/1000 ranked rows because the live rows came from Railway's 2026-06-02 pull. Backfill therefore (a) downloads the latest `ranked_keywords.json` per domain via the existing `/artifact` endpoint, and (b) merges keyword→KD maps across ALL artifact dates (newest wins) so keywords that dropped out of the latest pull keep their last-known KD. Result: 1,482 rows backfilled across 8 audits (IMA 1013/1099; the remainder are keyword_research seeds and synthetic keywords, correctly NULL).

**Proven ceiling = second-highest owned KD, not "≥2 at that exact KD".** The source methodology says "highest KD at which you have ≥2 Pos-≤7 rankings"; with near-continuous KD values, exact-level counting almost never reaches 2. Implemented as: sort owned (pos 1–7, non-brand, KD non-null) KDs descending, ceiling = the second value — equivalent fluke-guard semantics (one outlier win can't set the bar). Consequence: cluster ceilings mathematically can't exceed the site ceiling under this definition (subset second-max ≤ superset second-max); the per-cluster value is still the useful one for stretch-target flagging. Brand keywords are excluded — they prove brand recognition, not topical authority.

**Workstream A outputs are disk artifacts, not DB tables — deliberately.** The extraction map suggested new tables/JSONB for A1/A2 output. Deferred: thresholds (position band, growth >2.0, impressions ≥50, evaluative-language list) are untuned, and schema churn before signal validation is backwards. Scripts write `audits/{domain}/analysis/{name}.{json,md}`; DB persistence + dashboard surfacing move to FOLLOWUPS, to be picked up once a couple of monthly cycles confirm the signal. A2 deliberately makes its own paginated GSC pull (rowLimit 25000 + startRow): GSC sorts query×page rows by clicks DESCENDING non-configurably, so the existing top-1000 pull systematically excludes exactly the zero-click rows the analysis hunts.

**Signal-quality verification (IMA/SMA/Weiser).** Weiser: cold-start ceiling, 0 candidates, graceful no-GSC skip (sparse-data path confirmed). IMA: site ceiling KD 49; informational first-aid clusters punch 47–49 while local commercial training clusters own only KD 0–11 — matches intuition and explains why KD-30 commercial gap recommendations are stretch targets. 10 re-eval candidates, all pre-existing pages in clusters grown 2.6–14.7× (lower-bound growth — pages predate tracking). A2 flagged 454 zero-click queries from 60k rows; persona-variant fan-out queries ("how to first aid for men / graduate students / healthcare workers") are textbook LLM probe patterns, and DataForSEO LLM Mentions (10 head keywords only) confirmed citations on the same pages — the two signals triangulate as designed. Known noise class documented: pasted quiz questions ("select one answer…") match the signature; left in deliberately (they are still zero-click retrieval surface) but flagged for tuning.

---

### 2026-06-12 — KB extraction Workstream B: SERP composition (B1), BoF length directive (B2), video carousel ride-along (C3)

**B1 classification is deterministic, not LLM-judged.** Each top-10 SERP slot is classified by rule: weak_slot from a fixed forums/UGC/aggregators/directories list (+ forum/blogspot/wordpress patterns), direct_competitor from the audit's own `industry_competitor` rows in `audit_topic_competitors`, everything else authority. An LLM classifier was rejected: per-gap-run cost and nondeterminism for a judgment that is essentially a domain lookup, and misclassification of an obscure authority as "authority" (the default) is the safe failure direction. The weak-slot list lives in `src/agents/gap/serp-composition.ts` — extend it there when a new aggregator shows up in artifacts.

**Effective KD thresholds taken verbatim from the knowledge base (≥4 weak ×0.7, ≤1 weak ×1.3), unvalidated — deliberately.** These are the source methodology's numbers; we have no local data to tune them yet. The raw classification is persisted per run in `research/{date}/serp_composition.json` (disk-first pattern) so threshold tuning has ground truth when a few cycles accumulate. The block injects per-keyword WITHIN_REACH/STRETCH verdicts using cluster ceiling first, site ceiling fallback, +5 headroom (also from the source methodology) — IMA verification showed exactly the intended behavior: "phlebotomy courses near me" (KD 11 → effective 14) marked STRETCH against its cluster ceiling of 7 despite a site ceiling of 49, and the model's recommendation pivoted to long-tail variants. The site ceiling alone would have called it easy — the per-cluster gating is the entire point.

**SERP calls are capped and spread: 12 keywords, max 2 per cluster, live/advanced.** Advanced (not regular) because item-type detection (video carousel, C3) only exists there and the marginal cost over regular is small at depth 10 (~$0.024 per gap run total, cost-logged). Selection is revenue-ranked (`delta_revenue_mid`, volume tiebreak) but cluster-capped so one big cluster can't consume all lookups. Per-keyword failures and the whole block are non-fatal — pre-036 audits (empty KD column) degrade to no block, verified live on Weiser.

**B2 fires only on `buyer_stage ~ decision` + commercial/comparison coverage_role — Michael-sourced pages exempt by construction.** `buyer_stage` is only populated by cluster-strategy/operator paths (507 of 567 rows are NULL), so the directive can't accidentally constrain pillar/hub pages. The Pam prompt's standing rule ("let intent drive length, not word counts", quality rule 8) was amended with an explicit exception rather than weakened — the directive is the only binding word count in the system, and Oscar's playbook §4 mirrors it as a hard 800 ceiling with "cut educational depth, never proof/differentiation/conversion path." Live verification (weiser `/towing-services/weiser-id`): Pam emitted "Target Word Count: 600 words. Hard ceiling: 800" and scoped the outline accordingly. Found and fixed in the same pass: `extractWordCountTarget()` only parsed the outline section, so the metadata-block directive line never persisted — now falls back to metadata; the column was null for every brief whose word count Pam stated only in metadata.

**C3 shipped as prompt-only — the detection plumbing already existed.** The extraction map's claim that no SERP data exists at brief time was stale: Pam has fetched `live/advanced` per brief (PAA/features/organics) since the SERP-enrichment work, including `has_video`. C3 therefore cost zero new calls on the Pam side (prompt instruction: VideoObject stub only when the brief actually recommends video) and rides B1's calls on the Gap side (carousel flag per opportunity keyword). Oscar's existing `<!-- VIDEO: -->` recommendation system was extended, not replaced: a flagged carousel lowers the bar for recommending video and carries Pam's VideoObject stub through. Note: zero video carousels appeared across all 24 verification lookups (boiseheatingair, IMA) — local-service SERPs may rarely have them; the flag path is unit-tested, live validation pends a keyword set that actually triggers it.

---

### 2026-06-12 — KB extraction Session 3: ceiling injection (analytics→agent bridge) + C1 extended titles + C2 position-band linking

**The ceiling now reaches every agent that makes difficulty judgments.** Session 1 computed the proven ceiling; Session 2 used it inside Gap; this session closes the loop for Strategy Brief, Michael, and Cluster Strategy via shared plumbing (`fetchProvenCeiling()` + `buildCeilingPromptBlock(result, instruction, {focusClusterKey})`) — one data block, per-consumer instruction text. All injections are non-fatal try/catch. Three deliberate behaviors: (1) Phase 1b runs before Jim, so FIRST runs get an empty block and RE-runs get the prior run's ceiling — correct, since a re-run's strategy should be informed by measured performance; (2) cold-start sites (<15 owned) get a cold-start note ("treat moderate+ KD as stretch") instead of silence — verified on boiseheatingair, where the brief turned it into "the 4-ranking footprint is not a valid anchor... treat as cold-start domain"; (3) Cluster Strategy gets `focusClusterKey` so a cluster sees only its own ceiling plus the site fallback. IMA verification produced the exact intended downstream framing: "downstream analysts should use cluster ceilings, not the site ceiling, as the operative authority bar." Michael's boiseheatingair blueprint enforced within-ceiling-first sequencing with explicit "(stretch target — requires authority building)" markers in prose sequencing notes (NOT in slug cells — checked).

**Oscar's system prompt contradicted B2 — caught and fixed.** The playbook got the BoF ceiling rule in Session 2, but Oscar's system-prompt.md still said "Word count targets as hard floors or ceilings" are not to be followed, twice. Both spots now carry the explicit Bottom-of-Funnel exception. Lesson recorded: Oscar's behavior contract lives in TWO files (system-prompt.md + seo-playbook.md) — a directive added to one must be checked against the other.

**C1 extended titles: additive field, not a replacement.** The conditional `Extended Title Tag` (3+ intent variants, 150–200 chars, 2–3 segments) is a NEW metadata field; the ≤60 Meta Title keeps its format and its `execution_pages.meta_title` persistence path (`extractMetadataField` matches `**Meta Title:**` exactly — verified no collision). Capped at 200 chars vs the knowledge base's 250 — first-deployment conservatism. Oscar records both titles; the extended one is the intended `<title>` element.

**C2 position data comes from gsc_page_snapshots, displayed only when available.** `formatRelatedPagesSection` adds the Position column ONLY when a positions map is passed (legacy callers/tests unchanged). Routing is preference-among-eligible, never an eligibility gate — semantic similarity still decides what CAN be linked; position decides what SHOULD be. The 4–20 band preference, 1–3 navigation-only rule, and ≤25 cap come from the knowledge base's reasonable-surfer material; thresholds unvalidated locally (same posture as B1 — artifacts accumulate, tune later).

**Found during verification, logged not fixed:** `parseBlueprintMarkdown()` counts Michael's Buyer Journey Coverage table rows as invalid url_slug rows (~38% "corruption" on every blueprint with those tables, one wasted retry Sonnet call per Michael run). Pre-existing — confirmed in weiser's 2026-06-11 blueprint. FOLLOWUPS entry with the fix (skip tables with no slug column).

---

### 2026-06-12 — KB extraction Session 4: Workstream D (thin starter pages + individual FAQ architecture) + blueprint parser fix

**Trigger client: forgegrowth.ai.** Workstream D was deferred "until a new client with near-zero authority comes in" — forgegrowth.ai is that client (0 proven top-7 rankings), with a site redesign pending and a full pipeline re-run planned after it ships. Implemented now so that re-run uses starter-page architecture out of the box.

**page_mode lives in page_brief, not a new column — no migration.** The extraction map suggested `execution_pages.page_mode` as a column. `page_brief` already carries Michael-owned per-page metadata (role, coverage_role), and the one hazard of page_brief — syncMichael wholesale-overwrites it on re-runs — is exactly the right behavior here: Michael owns the mode and each re-run re-decides it. Contrast with `related_pages` (own column BECAUSE the overwrite would destroy it). Promote to a real column only if the dashboard ever needs indexed filtering (FOLLOWUPS).

**Low-authority mode triggers on cold start, not on a block's presence.** The Michael starter block fires when `ceiling.cold_start` (<15 owned, including zero) — independent of the ceiling block, which renders empty at zero owned keywords. The Mode column is instructed via the conditional context block rather than the static template, so non-low-authority clients keep the 8-column table format and never see starter semantics. Verified on boiseheatingair (4 owned): Michael added the Mode column everywhere, kept `full` for proven-ranking pages with explicit reasoning ("client ranks pos 6 — proven ranking justifies full content investment"), marked untested keywords `starter`, and produced individual `/faq/{question-slug}` starter rows (D2) instead of accordion sections. Chain verified end-to-end: parser → page_brief.page_mode (19 starter / 9 full) → Pam starter brief (exact-question H1, "Target Word Count: 300" persisted, exactly one pillar link) → detect-starter-expansion verdicts.

**Oscar's dual-file pre-flight check applied for real this time.** Grepped both Oscar files before changing anything: three conflicts found and reconciled — the two word-count exceptions (extended to Starter Page Directive) and "No thin sections" (starter pages are now the named deliberate exception). The check is documented in configs/agents/README.md.

**Blueprint parser extracted to src/pipeline/blueprint-parse.ts and the false-corruption bug fixed.** Buyer Journey Coverage tables (`| Buyer Stage | Coverage | Pages | Notes |`) were being parsed as page tables — the plural "Pages" list column matched the `'page'` header fallback, and its comma-separated values rejected as corrupted slugs (~38% constant false rate, one wasted retry Sonnet call per Michael run, and a desensitized corruption signal). Fix: skip tables with a stage column; never treat a plural "pages" column as the slug column. Extraction was forced by testability (scripts/ is outside the tsc rootDir and vitest include — sync-to-dashboard has pre-existing type errors that an src test import would have surfaced); sync-to-dashboard re-exports for compatibility. Verified: the boiseheatingair blueprint that previously rejected 20 rows now parses 33/33 clean, and the new Michael run produced zero corruption warnings and zero retries.

---

### 2026-07-02 — GSC page dedup sums collided URL variants instead of discarding them

**`fetch-gsc-data.ts` was undercounting site GSC totals.** GSC returns multiple URL variants for the same page (trailing-slash, www/non-www, http/https), which `normalizePageUrl` collapses to one path. The upsert dedup kept only the higher-click variant and **discarded the other's clicks/impressions** (`if (!existing || p.clicks > existing.clicks)`), so `gsc_page_snapshots` (which the dashboard Site Traffic section sums) reported less than the true GSC total. Found via IMA June 2026: the homepage `/` appeared as two variants (632 + 557 clicks); the dashboard stored 3,564 clicks / 573,582 impr while the true total was 4,121 / 582,493 — the latter matching GA4's Search-Console-integration report exactly (GA4 has no native clicks/impressions/position; that report is GSC data piped into GA4).

**Fix: sum collided pages** — clicks and impressions add, position is impression-weighted across the variants, CTR recomputed from the summed totals, top_queries kept from the richer variant. Only the DB write changed; `gsc_data.json` still stores every raw page row. After the fix + IMA May/June re-backfill, the dashboard matches the authoritative GSC/GA4 figure. Residual gap to a hand-pulled GSC UI number is normal UI-vs-API variance (anonymized rows, date-window/finalization), not a pipeline defect.

---

### 2026-07-04 — Page Optimizer + cluster editability + cornerstone ingest: opening the closed loop for established sites

**The strategic diagnosis this session was built on: the pipeline is creation-biased.** Michael already classifies pages create/optimize/differentiate and sync sets priority by it, but downstream there is exactly one content behavior — write from scratch (full or starter). `action_required='optimize'` rows dead-end. Established sites (the "lingered in obscurity or strong-but-needs-AI-fortification" case) got a technical fix list and nothing else. The three features shipped here are deliberately the FOUNDATION for a Phase 2 optimize-mode content path (see docs/plans/optimize-mode-phase2.md), not a complete solution.

**Page auditor is two layers, and the split is the point.** Mechanical scoring (`score-page.ts`) is code, not LLM — computed for every crawled page at syncDwight time (`optimization_profile`/`optimization_score`, migration 037) so the site-wide worklist costs nothing per view. The LLM (one Sonnet call) runs only on user-triggered single-URL deep dives. Wholesale scoring of 500 pages through Claude was rejected on cost; the deterministic layer decides where the LLM spend goes.

**Single-page crawl is native fetch + cheerio, not DataForSEO.** OnPage API is whole-domain (`target: domain`), costs per crawl, and is async-poll — wrong shape for an interactive one-URL audit. cheerio added as the first HTML-parser dependency in the repo. Deliberate side effect: parsing raw pre-JS HTML measures exactly what a non-rendering AI crawler sees, which is itself an Aleyda Layer-2 "Accessible" signal.

**Agent-readiness checks are code-verified, replacing prompt-asserted.** The retired Dwight §10.4 scorecard had Claude guess PASS/FAIL on `.well-known/mcp.json`; `agent-readiness.ts` actually fetches robots.txt/llms.txt/mcp.json (with a soft-404 guard — SPAs return 200 HTML shells for any path; the guard caught exactly this on idahomedicalacademy.com during the smoke test) and organizes checks by the Aleyda 3-layer framework per docs/ai-readiness-framework-reference.md. The old §10.4 prompt/sync path is untouched — this is the intended rebuild running alongside, page-scoped.

**Cluster editability is topic-first; keyword-level locks were cut on purpose.** Matt's framing: entity optimization/topical authority is the objective, keywords are directional evidence. So the edit surface is topics (create/rename via `is_manual`/`edited_at`) and page↔topic assignment (`execution_pages.assignment_locked`); `audit_keywords.canonical_key` stays fully pipeline-derived, and the audit_keywords UPDATE RLS policy was deliberately NOT added. The original plan had `cluster_locked` on keywords — cut because it added machinery to the most fragile code (delete+reinsert rebuilds) for the thing humans curate least. If merged-topic durability is ever needed, revisit with an overlay ops table, not row locks.

**Preservation strategy: exclude-from-delete for manual rows, restore-after-insert for edited fields.** Manual clusters are excluded from the rebuild DELETE (both in `rebuildClustersAndRollups` and syncJim's pre-delete) and win canonical_key collisions against derived inserts (the `(audit_id, canonical_key)` unique index enforces it; derived record is skipped with a log line). Edited display fields ride the existing statusMap restore pattern rather than a new mechanism. Manual clusters are intentionally EXCLUDED from `audit_rollups` totals — they represent existing structure, not derived opportunity.

**Cornerstone content is an overlay INPUT table, not edits to derived tables.** `cornerstone_pages` (migration 039) is dashboard-written/pipeline-read — the inverse of every other table's flow — so pipeline reruns can never destroy it and it can never corrupt derived state. The upload is a Screaming Frog export whose manually-inserted section rows ("ABOUT (ENTITY OPTIMIZATION)", "FIRE & EMS", …) ARE the user's declared architecture; the parser is section-aware and the sections become declared-silo anchors in Michael's prompt with explicit reconcile-don't-override rules (conflicts surface in the Executive Summary). Cornerstone rules live in the injected context block, not michael/system-prompt.md — same idiom as the gap/platform sections.

**Live-DB verification now goes through the Management API.** `supabase db query` does not exist in CLI v2.75.0 (CLAUDE.md's migration protocol was unrunnable as written — now corrected); no DB password is on file. Working channel: `POST https://api.supabase.com/v1/projects/hohuimkcpihdufunrzvg/database/query` with a personal access token (Matt issues 1-day temp tokens). This session's pg_policies verification found real drift: `audit_clusters` had a live owner-scoped UPDATE policy in no migration file (so hide/unhide always worked — drift, not the suspected silent no-op), and the `*_write_service_role` ALL-to-authenticated policies are dead no-ops (`auth.role()='service_role'` predicate). Migration 038 codifies the drifted policy via DROP+CREATE so tracked SQL is the source of truth.

**CLAUDE.md corrected on two false claims:** cluster mutations were documented as "via edge functions" but hide/unhide/delete/add-pages are direct client writes under RLS (now the sanctioned pattern for simple edits; edge functions only where pipeline work must be triggered), and the `supabase db query --linked` protocol reference was replaced with the Management API method.

---

### 2026-07-05 — Page-audit deliverable contracts hardened (Matt's first-run feedback)

**@graph "complete replacement" is only safe as a verified superset — preservation moved from prompt to code.** The prompt told the LLM to "preserve any correct data from the current JSON-LD," which made preservation prompt-asserted — the same failure class as the retired §10.4 scorecard, rebuilt one level up. Fix: `src/agents/page-audit/schema-diff.ts` computes an entity-by-entity (keyed by normalized @id, type-fallback for IRI relocations), property-by-property diff of the proposal against the live schema; `findings.graph_schema.diff = {preserved, added, modified, removed}` and removals render as sign-off warnings in the dashboard before anyone pastes. Kept the full-replacement artifact (a pure diff would push the merge onto the client's dev — reintroducing the error at their end); the diff is the verification AND the shows-the-work client artifact. `page_snapshot` now persists the raw parsed JSON-LD as the re-run baseline (was only a count).

**WebMCP starter contracts must not claim unverified endpoints.** First live run proposed a tool POSTing to a WordPress contact form — a code-state claim about an endpoint nobody verified (CMS forms reject programmatic POSTs: nonces/CSRF/honeypots), and a manifest advertising tools that 400 is NEGATIVE agent-readiness signaling. Prompt now mandates "manifest + endpoint implementation required" framing and forbids asserting existing-endpoint behavior.

**Mechanical score relabeled "Technical Floor" — it is a floor detector, not an optimization ranking.** IMA homepage: 14/14 mechanical checks while the deep dive correctly flagged the H1 as the top extractability fix. On mature sites the score-sorted worklist inverts. Deliberately did NOT blend readiness pass-rate into the sort — readiness data only exists for deep-dived pages and sorting on a ~95%-null column is fake precision. Honest label now; blend when findings lifecycle provides per-page severity (FOLLOWUPS).
...
### 2026-07-07 — Outreach draft queue: drafts in Gmail, never autonomous sending

**The pipeline generates outreach emails but never sends them.** `generate-outreach-email.ts` materializes copy as a draft in matt@forgegrowth.ai's Gmail Drafts folder; Matt reviews and sends manually. Human review is the product, not a compromise: the prospect list is tens of curated SMBs (review costs minutes), a hallucinated claim in a cold email is a brand-level failure, and drafts-from-Matt's-real-account sidesteps the entire deliverability apparatus (dedicated sending domain, warm-up, SPF/DKIM/DMARC, CAN-SPAM unsubscribe plumbing) that autonomous cold email would require. No `drafts.send` or `messages.send` call exists anywhere in the codebase — `gmail.compose` is the narrowest scope that can create drafts (no drafts-only scope exists), and that gap between scope and usage is documented rather than pretended away.

**Gmail auth is keyless DWD via signJwt, reusing the existing impersonation chain.** Org policy `iam.disableServiceAccountKeyCreation` rules out SA key files; the established ADC → `generateAccessToken` chain (GA4/GSC) can't act AS a Workspace user. The bridge: IAM Credentials `signJwt` signs a domain-wide-delegation assertion as `fg-analytics` (same `roles/iam.serviceAccountTokenCreator` grant, no new IAM), exchanged at the token endpoint for a user-delegated token. One-time Workspace admin authorization required (docs/GMAIL_DWD_SETUP.md).

**Recipient-facing copy draws only from `prospect-narrative.md` + `scope.json` — never `scout_markdown`** (same leak discipline as the 2026-07-05 get_share_report fix), and the prompt carries an explicit grounding rule: cite only provided facts, never estimate missing numbers. The $2,500/mo fit threshold routes to a `courtesy_note` variant below threshold (poor-fit honesty as referral play) — warn, never block, consistent with the share gate.
...
### 2026-07-08 — Daily Prospector: autonomous top-of-funnel, full-auto to drafts with a 4-week kill criterion

**Why full-auto instead of an approval queue:** Matt's self-identified constraint is that he won't prospect manually ("if I have to prospect, I'll struggle forever" — but closes well once talking). A qualification queue requiring his approval before Scout spend would recreate the exact manual step the system exists to remove. Scout cost turned out to be trivial (~$0.07/run DataForSEO, verified from SMA/IMA scope.json), so the usual reason to gate — spend — doesn't apply. The safety net is the morning digest (drafts ready + generated configs for spot-checking), not a pre-approval gate. Outreach remains draft-only per the 2026-07-07 decision; nothing sends autonomously.

**Trimmed v1 (evaluate-then-trim):** 3 scouts/day (not the 5–10 first floated — reviewing 10 configs+drafts daily is a chore that kills the habit; 3 good ones is sustainable), SERP-only discovery (no Maps until hit quality is proven), single-touch emails (no follow-up sequences yet). **Kill criterion:** re-evaluate ~2026-08-05. Drafts piling up unsent → kill or redesign the send step. Drafts sent but ~0 replies → fix message/sequences, not prospecting. Working → consider Maps discovery, follow-up sequences, gmail.send digest.

**Treasure Valley is hard-excluded in three layers** (seed geos contain no Boise-area metros; candidate domains containing boise/nampa/caldwell/meridian/treasurevalley are dropped; qualifier hard-zeros anything serving Treasure Valley): Matt's prior agency operates there and toe-stepping is a business risk, not a preference. The triple redundancy is deliberate — a single filter failing (e.g. Sonnet putting a TV metro in target_geos, which is also explicitly validated) must not produce outreach into that market.

**Rank window 11–50 as the discovery signal:** businesses on pages 2–5 for money keywords are real (they rank at all) but demonstrably losing search — which is exactly the pitch. Rank 1–10 businesses don't need the pitch; unranked businesses are unverifiable. Within the window, better rank sorts first (closer to page 1 = more established = better prospect).

**Backlog resurfacing over discovery-only:** the first dry run found 173 candidates in one day against a 15/day qualify cap. Overflow rows persist as `discovered` and compete (by rank) for future qualify slots; prior `qualified` rows compete (by score) for future scout slots. Without this, dedup would permanently bury ~90% of discovered candidates. Consequence: the pool self-feeds for months; discovery breadth is not a constraint.

**Auto-authored prospect configs are the accepted risk:** Sonnet writes `prospect-config.json` (previously hand-written) from homepage + SERP evidence. Bad topic_patterns/geo → confidently wrong Scout data → wrong email. Mitigations: structural validation before any Scout run (geo_type enum, non-empty target_geos, ≥3 topic_patterns, TV leak check — invalid config marks the candidate `error` and skips the Scout entirely), and every generated config appears in the digest for spot-checking during the first weeks.

**Digest is a Gmail draft addressed to the sender, not a sent email:** DWD authorizes `gmail.compose` only (2026-07-07 decision); adding `gmail.send` for the digest alone wasn't worth widening the scope in v1. Matt opens Drafts daily to send outreach anyway, so the digest lives where the workflow already is. Upgrade path documented if notification matters later.

---

### 2026-07-09 — Prospector footprint locked to Matt's target regions; Nampa/Caldwell un-excluded from the Treasure Valley ban

**Footprint (Matt's directive):** Idaho (now including Nampa and Caldwell), eastern Oregon (Bend, Pendleton, La Grande, Baker City), eastern Washington (Spokane, Kennewick, Yakima, Walla Walla), western Montana (Missoula, Bozeman, Kalispell, Helena, Butte), western Wyoming (Jackson, Rock Springs, Cody, Evanston), northern Utah (Ogden, Provo, Logan). Removed: all western WA/OR metros (Tacoma, Bellingham, Olympia, Vancouver WA, Portland, Salem, Eugene, Medford), Nevada entirely, St George, Billings, Great Falls, and **Salt Lake City** — Matt considers SLC as agency-saturated as Boise. The rationale is avoiding saturated agency markets, not geography per se.

**Nampa/Caldwell un-excluded (supersedes part of the 2026-07-08 "Treasure Valley hard-excluded" entry):** Matt explicitly lifted the ban for Nampa/Caldwell while keeping **Boise and Meridian hard-excluded** (saturated market). All three guard layers were narrowed rather than removed: domain-substring list is now `boise|meridian|treasurevalley` (a domain *branded* Treasure Valley almost certainly serves Boise), the qualifier hard-zero now targets "based in or primarily serves Boise or Meridian" with an explicit Nampa/Caldwell carve-out, and the config validator's leak regex is `/boise|meridian/i`. The `serves_treasure_valley` JSON field name is retained for schema continuity in `prospect_candidates.qualify` — its meaning is now Boise/Meridian specifically (documented in the qualifier prompt).

**Seeds are the footprint source of truth for the queue, not just discovery:** the backlog (`discovered`) and scout-pool (`qualified`) pulls now filter `geo->>metro` against the current seed metros (same leave-buried convention as removed verticals). Without this, the 07-08 Medford/Eugene/SLC cohort (13 of 20 qualified rows, ~127 of 158 discovered rows) would have kept winning scout slots for weeks after the footprint changed. Buried rows are dedup-known (never re-discovered); revive by re-adding the metro or a manual status flip. The Long's Heating draft (Medford) predates the footprint decision — Matt decides whether to send it.

**Ops note (first cron firing):** the 2026-07-09 12:00 UTC Railway firing crashed instantly — the cron service's start command had been saved as `npm tsx …` instead of `npx tsx …` (npm has no `tsx` subcommand). Matt fixed the start command in the dashboard the same morning; the schedule trigger itself is proven (container started exactly on the tick). The 07-09 run was executed locally instead.

---

### 2026-07-09 — Pam drain-loop + DB-fallback context (over a Railway volume)

**The stuck-briefs bug:** queuing N pages in a burst stranded N−1 at `pending`. `generate-brief.ts` fetched pending rows ONCE at startup (docstring said "polls"; it didn't), and the server's per-domain `inFlight` guard 409'd every trigger POST that arrived while the first worker was alive — the dashboard swallows the 409, so nothing ever claimed the later rows. 8 Weiser pages hit this on 2026-07-09. Fix: the worker drain-loops until the pending queue is empty (25-round cap; failed rows go to `status='failed'` so they can't spin it), with a 5s grace re-check before exit for the residual insert-after-final-check race. The `inFlight` guard is unchanged — it's correct once the live worker is guaranteed to pick up latecomers.

**DB fallback over Railway volume for degraded context:** ~~Railway mounts NO volumes on any Forge OS service~~ **CORRECTION (same day, from runtime logs + `railway volume list`):** the MAIN service does have `nanoclaw-volume` mounted at `/app/audits` — the earlier check misread the status JSON's empty `buckets` field as "no volumes". The cron services have no volume, and local runs never see Railway-generated artifacts — so the degraded-context problem is real for those contexts (the 07-09 stuck-brief drain ran locally and hit it), just not for the main service across redeploys. The DB-fallback choice stands, for the corrected reasons: volumes are per-service (the cron services can't share the main one), local runs need the fallback regardless, and Supabase already durably holds the artifacts (`agent_architecture_blueprint.blueprint_markdown`, `audit_snapshots` jim rows — verified 68/68 populated). Disk stays the primary source (exact file the run produced); DB is the fallback.

**Known remaining gap:** `strategy_brief.md` (Pam context 7b, Visibility Posture + Architecture Directive) has no DB home — deliberately left degraded rather than adding schema for it in this fix. If its absence shows up in brief quality, add a `strategy_brief_markdown` column (or audit_snapshots row) written by Phase 1b and extend the fallback pattern.

---

### 2026-07-09 (later) — strategy_brief_markdown closes the last Pam disk-context gap (migration 046)

Follow-up to the same-day drain-loop entry: the "known remaining gap" is closed. `strategy_brief_markdown TEXT` was added to `audit_snapshots` rather than a new table because the table already models per-agent payload rows and carries the exact unique key needed (`audit_id, agent_name, snapshot_version`). The `strategy-brief` row pins `snapshot_version=1` so the upsert is latest-wins — unlike dwight/jim/gap rows there is no value in historical brief versions for Pam's fallback, and version-incrementing would require the writer to know the prior max. Writer is `strategy-brief.ts` at generation time (not sync-to-dashboard) so every caller — run-pipeline.sh Phase 1b, QA retries, manual runs — persists it with no extra sync step; upsert failure is non-fatal since disk remains the primary source for the in-flight run. Note the pre-existing naming split: `agent_runs` logs this phase as `strategy_brief` (underscore) while `audit_snapshots` uses `strategy-brief` (hyphen, matching the documented phase list) — kept as-is to avoid touching historical `agent_runs` rows. Backfilled from local disk for the 4 domains that had artifacts (boiseheatingair, forgegrowth.ai, IMA, SMA); Weiser and others never had a local/durable copy and populate on their next Phase 1b run.

### 2026-07-10 — Thin-starter mode removed; comprehensive-by-default, with county geo tier and self-grounded geo facts

Three linked changes, all driven by the Weiser towing build where two county pages (Adams, Valley) came out as thin ~200-word "starter" pages and one draft placed Cambridge (a Washington County town) in Adams County.

**(1) Thin-starter mode removed.** The LOW-AUTHORITY MODE block (`pipeline-generate.ts`, gated on a cold-start proven ceiling <15 top-7 rankings) instructed Michael to mark untested-keyword pages `starter` and produce 200–400-word test pages. Removed because the cost/benefit does not hold outside programmatic-scale SEO: by the time Oscar runs, all upstream research/architecture cost is already sunk, so "starter" saves only the length of one LLM generation — cents — while a thin page underserves every goal (local relevance is the named locales the starter constraint strips out; a thin page may fail to earn the GSC impressions the lifecycle waits for, confounding the very signal; LLM answer engines have nothing to extract). The starter lifecycle is a legitimate tactic for thousands of demand-dead geo/keyword permutations, not a six-county service-area build. The proven-ceiling stretch-target *sequencing* (a separate mechanism sharing the same `ceiling` data) is retained. `page_mode` column, `detect-starter-expansion.ts`, and the `generate-brief.ts` starter branch are left in place but inert.

**(2) County geo tier added to base knowledge.** The city/metro and state geo blocks had only city/metro/state tiers — no county — which is *why* Michael defaulted to city-level pages for rural counties until a per-client operator directive patched it (the same root cause as the starter mistake: Michael lacked an encoded model of service-area granularity). Both blocks now instruct choosing city vs. county vs. state by population/competition/keyword signal, with county as the container for low-volume rural areas and significant towns named in-content. Generalizes the operator directive into base geo knowledge so future clients inherit the reasoning without a per-client hack.

**(3) Geo facts grounded by prompt discipline; web_search-in-Oscar tried and REVERTED same day.** The fix for the Cambridge-class error is deliberately *not* an operator-supplied town roster (that is the human buffer we are trying to eliminate). Oscar's prompt + playbook Dimension 3 reclassify geographic/factual specifics (town-in-county, road names, ordinance numbers) as facts requiring verification: state only from verified inputs (brief, schema, client profile), else refer generically or flag — never assert from memory. Dimension 3 previously *rewarded* confident local specificity and even modeled an invented ordinance citation ("Ada County Code 3-2-1"); that example is now caveated as correct only if the citation came from a real source. **Lesson learned the same day:** we first wired the server-side `web_search` tool directly into Oscar's generation call (restricted to en.wikipedia.org + census.gov). The grounding *logic* worked perfectly — a live Adams County run verified Council/New Meadows via Wikipedia, correctly excluded Cambridge, and refused to fabricate an unverifiable response-time figure — but holding a tool flipped the model into agentic narration: it described its searches and "exported a file" instead of emitting the HTML artifact, producing a draft with zero HTML in it. Reverted: Oscar's generation call is tool-free again. Design rule going forward: **the artifact-emitting call must never hold tools**; grounding belongs in a separate pre-generation step that does the web_search and injects verified locale facts into the brief (planned, not yet built). `webSearch.allowedDomains` support remains in `callClaude` for that future step and other phases. FAQ/PAA intent (formerly individual thin `/faq/` starter pages) now folds into on-page Q&A + FAQPage schema on the parent page (Michael Rule 18).

### 2026-07-10 — Publish→performance tracking: signal health via `error_message`, never `status`; add a zero-metric health check

Audit of the publish→performance loop found it inert for the only client with published content (Weiser): tracking disabled, no `analytics_connections` row, and every failure mode silent (GA4 errors caught "non-fatal" while the run still logged `status='completed'`; the `analytics_connections.status`/`error_message` columns existed but no code ever wrote them). Fixes shipped:

**(1) Health is signaled through `error_message`, deliberately NOT `status`.** The obvious move — set `status='error'` on a failed pull — is a trap: `getAnalyticsConnection()` filters `.eq('status','active')`, so an `'error'` row becomes invisible to every subsequent run — the connection never retries and never self-heals (a *worse* silent failure than the one being fixed). So `status` remains the on/off gate only; a transient pull failure writes a prefixed `error_message` (`GSC fetch failed …` / `GA4 fetch failed …`) and leaves `status='active'` so the next cron retries. Success clears the error **source-scoped** (`.like('error_message','GSC fetch failed%')`) so a GA4 success can't wipe a live GSC error — the two sources share one `error_message` column in v1 (per-source columns deferred to avoid a migration).

**(2) GA4 failure downgrades the run instead of masquerading as success.** `track-rankings.ts` now captures the GA4 catch into `ga4Error`, records it on the connection, and logs `agent_runs.status='completed_with_errors'` (+ `ga4_error` in metadata) rather than an unconditional `'completed'`.

**(3) A dedicated tracking-health check** (`scripts/check-tracking-health.ts`, called at the end of `cron-track-all.ts`, also standalone) detects the three silent states — recorded pull error, stale sync (GSC >14d / GA4 >40d), and the D9 headline: a published page older than a **10-day grace** with zero matching `gsc_page_snapshots` + `ga4_page_snapshots` rows. The 10-day grace exists so brand-new pages (GSC 'final' data lags ~3d + indexing) don't false-alarm — verified against Weiser's 4 pages published <10d ago, which correctly do NOT flag. Path matching emits leading- and trailing-slash variants to absorb the known slug/page_url drift. Surfaced via a `[TRACKING-HEALTH]` console block, a `tracking_health` `agent_runs` row, and (dashboard, cross-repo) a `useTrackingHealth()` badge on the Performance page reading `error_message`/sync-age. Pushed alerting (Gmail/Slack digest) deliberately deferred to v2 — v1 makes every state truthful and queryable first.

### 2026-07-13 — /scout-report gains a DB fallback: the internal Scout viewer was blind to every cron-scouted prospect

The Daily Prospector runs on a Railway **cron service**, which does not mount `nanoclaw-volume` (main service only — see the 2026-07-09 volume correction). Its scout artifacts land on the cron container's ephemeral disk, so the main service's `/scout-report` (disk-only) 404'd for every prospector-scouted prospect — the dashboard's internal Scout report page showed "Edge Function returned a non-2xx status code" for all of them, while the public share page kept working (it reads `prospects` columns via `get_share_report`, DB-only).

Fix follows the established disk-primary/DB-fallback pattern (same as Pam's blueprint/market-context fallbacks): when `audits/{domain}/scout/` has no date dirs, `handleScoutReport` serves `prospects.scout_markdown/scout_scope_json/prospect_narrative` (`scout_run_at` supplies the date). Specific-`file` requests stay disk-only — arbitrary artifacts have no DB home. A volume for the cron service was not considered for the same reasons as 2026-07-09: Railway volumes are single-service, and the DB already durably holds everything this endpoint returns.

### 2026-07-13 — Scout share surface upgraded in place; forgegrowth.ai hosting deliberately deferred

The hosted-prospect-report goal (one link per cold email → instrumented per-prospect page) shipped as an upgrade of the existing `app.forgegrowth.ai/share/scout/:token` surface rather than the new forgegrowth.ai build the original brief proposed. Why: the capability-URL page, structured Scout payload (`scout_scope_json` + `prospect_narrative`), and brand-palette styling already existed; app./www share the registrable domain so deliverability is identical; and the forgegrowth.ai move stays a template port later (same data contract) — FOLLOWUPS item. Migration 047 + edge function + dashboard + outreach changes; plan at `tmp/scout-share-upgrade-plan.md`.

Locked choices:
- **Timestamps over a status enum.** `share_expires_at` / `first_viewed_at` / `last_viewed_at` / `view_count` / `booking_intent_at` are the source of truth; expiry is computed at request time (no cron flips rows, no multi-writer status column). `outreach_status` stays the coarse manual marker and gains `'sent'` via the dashboard's one-click Mark-sent, which also restarts the 14-day expiry from the send (drafts sit; the clock shouldn't run while they do).
- **Single server-side checkpoint.** Access, expiry (410), view logging, and booking intent all live in `scout-config` — a future hard gate for a sensitive prospect is a change in one function.
- **Booking is a config value.** `/book/scout/:token` (SPA route) → `log_booking_intent` → 302-style redirect to the `BOOKING_URL` secret (Google Calendar, 24h–14d window). Calendar tools can't carry attribution through a booking; instrumenting the click on our surface recovers the intent signal. Swapping schedulers = changing one secret.
- **One-link rule reverses the no-links rule** (043-era) in `generate-outreach-email.ts`: the share URL exactly once, plain text, framed as their prepared report. The old rule was deliverability caution; a single same-domain link at ≤3 sends/day is normal business mail **provided root SPF + Gmail DKIM are published — they were NOT as of 2026-07-13** (only the `send.` subdomain SES/Resend records exist; DMARC is p=none). Sending link-bearing mail waits on that DNS task; drafting doesn't. The script also mints/replaces missing-or-expired tokens at draft time so a drafted link is never dead on arrival.
- **Bridge line in the Scout output contract.** The narrative call emits a trailing `BRIDGE_LINE:` parsed by `src/pipeline/bridge-line.ts` → `prospects.prospect_bridge_line`; the share page renders it as the seam between the prospect's data and the positioning layer (generic fallback when null). Positioning copy lives in `ScoutSharePositioning.tsx` from the one-pager copy spec, headline per Matt 2026-07-13: "Search marketing built as a system — measured in business outcomes, not vanity metrics."
- **Discovery bounding, not access control:** `X-Robots-Tag: noindex, nofollow` + `Referrer-Policy: no-referrer` on `/share/scout/*` and `/book/scout/*` via vercel.json headers; the token bounds access.
- Backfill: existing tokens got `share_expires_at = created_at + 14d` — the nine April-era links expired immediately (never sent to prospects; they also predate `prospect_narrative` and can't render the share page until re-scouted).

### 2026-07-16 — Scout share page adopts the design-system template; booking route and data guards deliberately deviate from the mock

The public share surface (`lovable-repo/src/pages/ScoutShareReport.tsx`, dashboard @ 93a8604) was rebuilt to match `tmp/scout-report-design-system/templates/scout-report/ScoutReport.dc.html` (full-bleed dark/cream bands, square corners, mono eyebrows, blue accent, new "one operator" positioning copy — this supersedes the 2026-07-13 one-pager copy spec in the deleted `ScoutSharePositioning.tsx`). Three deviations from the mock are intentional:

- **CTA keeps `/book/scout/:token`**, not the template's hardcoded `calendar.app.google` link — the internal route logs `booking_intent_at` and redirects to the same `BOOKING_URL` secret. Hardcoding the calendar would silently kill the attribution built in migration 047.
- **Low-volume guards:** the "monthly searches, top query" stat tile renders only at ≥100/mo and the keyword table only at ≥50/mo top volume. `top_opportunities` for thin prospects (e.g. byplumbing.com: two 10/mo rows worth ~$14) reads as weakness beside the addressable-revenue callout; fallback stats (striking-distance/gap counts) fill the grid instead. The template's bespoke per-client stats (e.g. "#1 for your business name") have no source in the share payload — position data for defending keywords never leaves `scout_scope_json.gap_summary` counts.
- **`formatMarket` now reads the pipeline's real `target_geos` shape** `[{state, metros[]}]` (legacy `{cities, state}` still handled). The old header expected only the legacy shape, so the market line ("Bend, Oregon") had never rendered for cron-scouted prospects.

Share-only components (`ProspectShareHeader`, `NarrativeSection`, `ScoutSharePositioning`) were folded into the page and deleted — nothing else imported them. Escaping-band H2 ("Invisible for six of the seven services you sell") derives from `service_coverage` (defending=0, gaps+weak>0), which can disagree by one with the LLM narrative's own count; data-derived wins.

### 2026-07-20 — Scout keyword identity is ONE canonical key + embedding join; the exact-string gap join was calling ranked keywords "gaps"

Matt caught outreach drafts contrasting word-order/morphology variants as if they were different rankings (Criterion's draft hook: #16 "idaho falls plumbing" vs #2 "plumber idaho falls" — Google treats these as the same query; to a prospect who knows SEO it reads as junior). Root cause was not Scout's weighting or the email writer: both sets (ranked keywords, opportunity candidates) were deduped *within themselves* by a token-sorted canonical key, but the gap-matrix cross-join matched **raw lowercase strings** (`rankedLookup.get(kwLower)`). A synthetic topic×geo candidate ("furnace repair medford") missed its ranked twin ("furnace repair medford or", #8) and was emitted as a not-ranking gap. Two different canonicalizers (`buildCanonicalKey`: suffix-only state strip; `buildOpportunityKey`: state-anywhere + plurals) compounded it. Every downstream consumer trusted the matrix: gap counts, `addressable_revenue_monthly` (phantom-gap volume double-counted), `top_opportunities`, the scout report tables, and the narrative — which then confidently wrote "you are not ranking for it" (Long's HVAC: claimed for "furnace repair Medford" while the same report's ranking table showed page 1).

Fix (pipeline @ this commit):
- **`canonicalKeywordKey()` is the single identity function** for scout keywords: state names/abbrevs stripped anywhere, `&`→`and`, plural + `-ing`/`-er` stemming (`plumbers`/`plumber`/`plumbing` → `plumb`), alphabetical token sort. Used by ranked dedup, opportunity dedup, the gap-matrix join, and the recipient-facing variant collapse. `buildCanonicalKey`/`buildOpportunityKey` are gone.
- **Join misses get a cross-set embedding pass** (`matchOpportunitiesToRanked()`, same 0.93 cosine threshold and cache as within-set dedup, non-fatal) so paraphrases the stemmer can't see also refuse to become phantom gaps. Ranked keywords consumed by an opportunity row are not re-added as standalone matrix rows.
- **Direction-of-error is deliberate:** over-merging understates the opportunity (conservative pitch); under-merging fabricates a gap the prospect can refute in 10 seconds. Stemming length floors (≥6 for -ing/-er) keep "water"/"sewer" intact; weird stems ("siding"→"sid") are harmless because keys are join-only, never displayed.
...
### 2026-07-22 — Outreach claim verifier (Bucket B): render escalation deferred, jsonb mirror over new table, deterministic subtraction over LLM rewrite

Pre-send verification of site-structure absence claims shipped per `docs/plans/FORGE_OS_OUTREACH_VERIFIER_SPEC.md` v1.2 and `docs/plans/outreach-claim-verifier-bucket-b-plan.md` (Matt approved both open calls 2026-07-22). Three non-obvious decisions:

**Render escalation deferred (spec §3 step 5 deviation, approved).** Playwright is not a dependency of this repo, and adding a headless browser + binary to the Railway image for a fallback path — inside a motion two weeks from its kill review — is premature weight. JS-only-nav / empty-fetch sites route to `UNRESOLVABLE` → Matt's manual queue instead; never-auto-pass is preserved exactly, only auto-*resolution* of that subset is lost. The verdict corpus records inventory quality per run, so the revisit trigger is empirical: if `nav_only`/`thin` inventories drive a meaningful share of `needs_review` flags, the escalation is a contained fast-follow inside `site-inventory.ts`. Companion rule (also approved): **only a sitemap-backed inventory can mint `ABSENT`** — a nav-only inventory that misses a page and then "confirms" an absence claim would re-commit the exact invalid inference (ranking-absence ≠ page-absence) the verifier exists to catch.

**Verdict corpus: disk-first artifact + jsonb mirror, not a table.** `audits/{domain}/outreach/{date}/verification.json` is the corpus entry (spec §1.5: the log is the real asset), but Railway container disk is ephemeral in the cron path — the very failure `scout_scope_json` already exists to absorb. So the artifact mirrors verbatim into `prospects.outreach_verification_json` (migration 050), latest-run-wins. A dedicated table is deliberately not created: thresholds must be validated against the corpus before the schema earns that commitment.

**Deterministic subtraction, no LLM near the body.** Cuts are sentence surgery keyed on the extractor's verbatim `asserted_text` (validated as an exact substring — a paraphrased span fails loud to `needs_review`); softens come from a fixed whitelist of absolute-presence phrasings ("zero presence" → "limited presence"). Nothing generates replacement text, so never-fabricate / never-rewrite-Matt's-voice (spec §0/§6) is structural rather than prompted, and the PRESENT-on-ambiguous-phrasing case can never drift into a ranking rewrite (the Bucket A boundary leak closed in spec v1.2). Whitelist misses route to Matt — seams go to the queue, never into a sent email.
