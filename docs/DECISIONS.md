...
**DB fallback over Railway volume for degraded context:** Railway mounts NO volumes on any Forge OS service (verified via CLI 2026-07-09) — container disk is wiped on every deploy, so Pam's disk-context readers (blueprint excerpt, research_summary striking-distance) silently degraded on Railway after any redeploy, and always degraded locally for Railway-run refreshes. A volume was rejected because Railway volumes attach to a single service (the cron services couldn't share it) and Supabase already durably holds both artifacts (`agent_architecture_blueprint.blueprint_markdown`, `audit_snapshots` jim rows — verified 68/68 populated). Disk stays the primary source (exact file the run produced); DB is the fallback.
